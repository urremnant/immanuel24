...
//	D10012	사랑부		
	...
	if ($attendCheckValue <> ""){
		$sql = $sql."and a.memberID in (select a.memberID from attendworshipcheck305 a, member b where a.memberID  = b.memberID  and b.rtdept1Code = '".$attendCheckDeptCode."' and a.week".$weekNum." ='".$attendCheckValue."' and baseYear = '".substr($worshipDate,0,4)."') ";
	}
	if ($mode == "Find"){
	...
	$page_rows = 100; // 페이지당 목록 수 20-> 100 2024-03-04 이제훈
	$page = $_GET['page'];
...
// 2024-03-04  부서 명단 목록 교역자 / 교사 / 렘넌트 
    $sql = $sql."ORDER BY left(newMemberID,1),rtdept2Name,korname LIMIT {$from_record}, {$page_rows}";
//	echo $sql;
	$result = mysqli_query($conn, $sql);
	...
	$str = ''; 
	if ($page > 1) {
		$str .= '<li class="page-item"><a class="page-link"  href="list305.php?page=1&mode='.$mode.'&Search='.$Search.'&SearchString='.$SearchString.'&churchareaCode='.$churchareaCode.'&rtdept1Code='.$rtdept1Code.'&rtdept2Code='.$rtdept2Code.'&attendCheckDeptCode='.$attendCheckDeptCode.'&worshipDate='.$worshipDate.'&attendCheckValue='.$attendCheckValue.'">처음</a></li>';
	}
...
function attendCheckToServer(attendType, worshipDate, weekNum) {
	...
	if (checkdatalist == "" ){
		...
		//alert(params);
		sendRequest("attendCheck305.php", params, attendCheckFromServer, "POST");
	}
}
function attendCheckFromServer() {
	if (httpRequest.readyState == 4) {
		if (httpRequest.status == 200) {
			...
			location.href="list305.php?attendCheckDeptCode=<?php echo $attendCheckDeptCode;?>&worshipDate="+document.rtsc.worshipDate.value+"&rtdept2Code="+document.rtsc.rtdept2Code.value+"&attendCheckValue="+document.rtsc.attendCheckValue.value;
		}
	}
}
...
function addReason(memberID, worshipDate){
	window.open("addReason305.php?memberID="+memberID+"&worshipDate="+worshipDate, "addReason", "status=no, menubar=no, scrollbars=no, resizable=no, width=700, height=400");
}
function editReason(idx){
	window.open("editReason305.php?idx="+idx, "editReason", "status=no, menubar=no, scrollbars=no, resizable=no, width=700, height=400");
}
function delReason(idx){
	window.open("delReason305.php?idx="+idx, "delReason", "status=no, menubar=no, scrollbars=no, resizable=no, width=400, height=300");
}
-->
</script>
    <!-- Main content -->
        ...
        <div class="row">
          <div class="col-12">
            <div class="card">
				  ...
				  <form class="form-horizontal" method ="POST" name="rtsc" action="list305.php">
						...
						<div class="form-group row">
							<label class="col-sm-1 col-form-label">예배날짜</label>
								...
								<?php
									# 현재 년도와 날짜를 구한다.
									$today = date("Y-m-d");
									$sql_worshipDate2 = "select worshipDate from attendbasedate where baseYear = '".substr($today,0,4)."' and worshipDate <= '".$worshipDate."' order by worshipDate desc";
// 2024-03-04 이제훈					$sql_worshipDate2 = "select worshipDate from attendbasedate where baseYear = '".substr($today,0,4)."' and worshipDate <= '".$today."' order by worshipDate";
...
?>
						<tr>
							...
							<td>
								<?php
									$sql_attend = "select week".$weekNum." from attendworshipcheck305 where memberID = '".$list[$i]['memberID']."' and baseYear = '".substr($worshipDate,0,4)."'";
									# echo $sql_attend;
							...
							<td>
								<?php
									...
									if (($row_attend['week'.$weekNum]=="C")||($row_attend['week'.$weekNum]=="D")){
										$sql_reason = "select count(idx) as cnt from absentreason305 where memberID = '".$list[$i]['memberID']."' and worshipDate = '".$worshipDate."'";
										# echo $sql_reason;
								...
								<?php
											...
											$sql_reasonContent = "select idx, absentreason from absentreason305 where memberID = '".$list[$i]['memberID']."' and worshipDate = '".$worshipDate."'";
...
<?php if (count($list) == 0) { echo '<tr class="text-center"><td colspan="10"><font color="blue">해당 데이터가 없습니다.</font></td></tr>'; } ?>
				  ...
				  <div class="card-footer">
					<div class="col-12 text-center">
						<form method ="POST" name="list" action="list305.php" onsubmit="return search()">
