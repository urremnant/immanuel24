        ...
        <div class="row">
          <div class="col-12">
            <div class="card">
				...
				<form class="form-horizontal" method ="POST" name="rtsc" action="totalPerson_backup_20211206.php">
				<div class="card-body">
						<div class="form-group row">
								...
								<div class="col-sm-2">
									<a href="javascript:sendit();"><button type="button" class="btn btn-primary">개인별 출석통계보기</button></a>
								</div>
						</div>
				</div>
				</form>
<?php
	if ($total_rtdept1Code <> "") {
		...
		# 현재 년도와 날짜를 구한다.
		$today = date("Y-m-d");
		$sql_worshipDate = "select baseYear, worshipDate from attendbasedate where baseYear = '".substr($today,0,4)."' and worshipDate <= '".$today."' order by worshipDate";
				...
				<div class="col-12">
						...
						<div class="card-body table-responsive p-0">
							<table class="table table-hover text-nowrap">
							  <thead>
								...
								?>
									<th><?php echo $row_worshipDate['worshipDate'] ?></th>
								<?php
									}
					...
					?>
								...
								?>
									<td>
										<?php
											$sql_weekNum = "select weekNum from attendbasedate where worshipDate = '".$row_worshipDate['worshipDate']."'";
											# echo $sql_weekNum;
											$result_weekNum = mysqli_query($conn, $sql_weekNum);
											$row_weekNum = mysqli_fetch_assoc($result_weekNum);
											$weekNum = $row_weekNum['weekNum'];
											# echo $weekNum;
											$sql_attend = "select week".$weekNum." from attendworshipcheck where memberID = '".$row_deptPerson['memberID']."' and baseYear = '".substr($row_worshipDate['worshipDate'],0,4)."'";
