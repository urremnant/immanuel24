...
//	echo $sql;
	...
	$str = ''; 
	if ($page > 1) {
		$str .= '<li class="page-item"><a class="page-link"  href="list.php?page=1&mode='.$mode.'&Search='.$Search.'&SearchString='.$SearchString.'&churchareaCode='.$churchareaCode.'&rtdept1Code='.$rtdept1Code.'&rtdept2Code='.$rtdept2Code.'&attendCheckDeptCode='.$attendCheckDeptCode.'&worshipDate='.$worshipDate.'">처음</a></li>';
	}
	...
	if ($start_page > 1) $str .= '<li class="page-item"><a class="page-link" href="list.php?page='.($start_page-1).'&mode='.$mode.'&Search='.$Search.'&SearchString='.$SearchString.'&churchareaCode='.$churchareaCode.'&rtdept1Code='.$rtdept1Code.'&rtdept2Code='.$rtdept2Code.'&attendCheckDeptCode='.$attendCheckDeptCode.'&worshipDate='.$worshipDate.'">이전</a></li>';

	if ($total_page > 1) {
		for ($k=$start_page;$k<=$end_page;$k++) {
			if ($page != $k)
				$str .= '<li class="page-item"><a class="page-link" href="list.php?page='.$k.'&mode='.$mode.'&Search='.$Search.'&SearchString='.$SearchString.'&churchareaCode='.$churchareaCode.'&rtdept1Code='.$rtdept1Code.'&rtdept2Code='.$rtdept2Code.'&attendCheckDeptCode='.$attendCheckDeptCode.'&worshipDate='.$worshipDate.'">'.$k.'</a></li>';
			else
				$str .= '<li class="page-item"><a class="page-link" href="list.php?page='.$k.'&mode='.$mode.'&Search='.$Search.'&SearchString='.$SearchString.'&churchareaCode='.$churchareaCode.'&rtdept1Code='.$rtdept1Code.'&rtdept2Code='.$rtdept2Code.'&attendCheckDeptCode='.$attendCheckDeptCode.'&worshipDate='.$worshipDate.'"><font color="red"><b>'.$k.'</b></font></a></li>';
				// $str .= '<span class="current">'.$k.'</span>';
		}
	}

	if ($total_page > $end_page) $str .= '<li class="page-item"><a class="page-link" href="list.php?page='.($end_page+1).'&mode='.$mode.'&Search='.$Search.'&SearchString='.$SearchString.'&churchareaCode='.$churchareaCode.'&rtdept1Code='.$rtdept1Code.'&rtdept2Code='.$rtdept2Code.'&attendCheckDeptCode='.$attendCheckDeptCode.'&worshipDate='.$worshipDate.'">다음</a></li>';

	if ($page < $total_page) {
		$str .= '<li class="page-item"><a class="page-link" href="list.php?page='.$total_page.'&mode='.$mode.'&Search='.$Search.'&SearchString='.$SearchString.'&churchareaCode='.$churchareaCode.'&rtdept1Code='.$rtdept1Code.'&rtdept2Code='.$rtdept2Code.'&attendCheckDeptCode='.$attendCheckDeptCode.'&worshipDate='.$worshipDate.'">맨끝</a></li>';
	}
            ...
            <div class="card">
				  ...
				  <div class="card-body">
						<div class="form-group row">
							...
							<div class="col-sm-2">
								<a href="javascript:sendit();"><button type="button" class="btn btn-primary">선택한 날짜 보기</button></a>
							</div>
						</div>
					</div>
				  </form>
				  <div class="card-body">
					<div class="col-12">
						<a href="javascript:allcheck()" class="btn btn-default">전체선택</a>
						<a href="javascript:alldischeck()" class="btn btn-default">선택해제</a>
						<a href="javascript:attendCheckToServer('A', '<?php echo $worshipDate;?>', '<?php echo $weekNum;?>');" class="btn btn-success">출석(대면)처리하기</a>
						<a href="javascript:attendCheckToServer('B', '<?php echo $worshipDate;?>', '<?php echo $weekNum;?>');" class="btn btn-info">출석(비대면)처리하기</a>
						<a href="javascript:attendCheckToServer('C', '<?php echo $worshipDate;?>', '<?php echo $weekNum;?>');" class="btn btn-danger">결석처리하기</a>
						<a href="javascript:attendCheckToServer('D', '<?php echo $worshipDate;?>', '<?php echo $weekNum;?>');" class="btn btn-warning">조퇴처리하기</a>
						<a href="javascript:attendCheckToServer('', '<?php echo $worshipDate;?>', '<?php echo $weekNum;?>');" class="btn btn-secondary">초기화하기(출결기록지우기)</a>
					</div>
				  </div>
