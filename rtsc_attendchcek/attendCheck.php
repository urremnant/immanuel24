<?php
	...
	for($i = 0 ; $i < $cnt ; $i++){
		$sql = "select COUNT(memberID) AS cnt FROM attendworshipcheck where memberID = '".$strCheckData[$i]."' and baseYear = '".substr($worshipDate,0,4)."'";
		# echo $sql."<br>";
		...
		# echo $total_count."<br>";
		if ($total_count == 0){
			$sql = "insert into attendworshipcheck (memberID, baseYear) values ('".$strCheckData[$i]."', '".substr($worshipDate,0,4)."')";
			# echo $sql."<br>";
			$result = mysqli_query($conn, $sql);
			$sql = "update attendworshipcheck set week".$weekNum." = '".$attendType."' where  memberID = '".$strCheckData[$i]."' and baseYear = '".substr($worshipDate,0,4)."'";
			# echo $sql."<br>";
			$result = mysqli_query($conn, $sql);
		}else{
			$sql = "update attendworshipcheck set week".$weekNum." = '".$attendType."' where  memberID = '".$strCheckData[$i]."' and baseYear = '".substr($worshipDate,0,4)."'";
		...
		if ($attendType==""){
			$sql = "delete from absentreason where memberID = '".$strCheckData[$i]."' and worshipDate = '".$worshipDate."'";
			$result = mysqli_query($conn, $sql);
		}
	}
?>