        ...
        <div class="row">
          <div class="col-12">
			...
			<?php
				...
				$sql = "select COUNT(homepage_admin_idx) AS cnt FROM homepage_admin where korname= '".$korname."' and mobile = '".$mobile."'";
				$result = mysqli_query($conn, $sql);
				$row = mysqli_fetch_assoc($result);
				//echo $row['cnt'];
				if ($row['cnt'] == 0){
					echo "<script>alert('일치하는 데이터가 없습니다. 성명과 핸드폰번호를 다시 확인해주세요.');self.close();</script>";
				}else{
					//6자리 난수생성
					$randomNum = mt_rand(100000, 999999);
					//echo $randomNum;

					$sql_random = "update homepage_admin set randomNum = '".$randomNum."' where korname= '".$korname."' and mobile = '".$mobile."'";
					...
					$sms_content	= "[렘넌트서밋위원회]로그인 인증번호는 ".$randomNum." 입니다.";	//전송 내용을 받음
					...
					echo "<script>alert('인증문자를 발송하였습니다. 인증문자를 입력하시고 로그인하여 주십시요.');self.close();</script>";
				}
				mysqli_close($conn);
			?>
            </div>
