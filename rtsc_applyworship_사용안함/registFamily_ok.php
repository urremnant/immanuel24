...
//	$memberGubun - R : 렘넌트, T : 임원/교사, P : 교역자
	...
	for($i=1;$i<=$familyCount;$i++){
		$myNo					= mysqli_real_escape_string($conn, trim($_REQUEST['myNo'.$i]));
...
//		echo $wheelchairYN."<br>";
		...
		$sql = "insert into member_familyinfo (familyID, churchareaCode, rtdept1Code, myNo, korname, birthday, churchPositionCode, mobile, gender, familyPosition, appYN, busUse, strollerYN, wheelchairYN, worshipPlace, useLanguage) values ('".$familyID."','".$churchareaCode."','".$rtdept1Code."','".$myNo."','".$korname."','".$birthday."','".$churchPositionCode."','".$mobile."','".$gender."','".$familyPosition."','".$appYN."','".$busUse."','".$strollerYN."','".$wheelchairYN."','".$worshipPlace."','".$useLanguage."')";
		//echo $sql."<br>";
