...
<?php
	...
	$sql = "select a.worshipDate, a.korname, d.korChurchPosition, b.korParishName, c.rtdept1Name, a.appYN, a.busUse from apply_worship a, churcharea b, rtdept1 c, churchPosition d where a.churchareaCode = b.churchareaCode and a.rtdept1Code = c.rtdept1Code and a.churchPositionCode = d.churchPositionCode and worshipDate = '".$worshipDate."' ";
	if ($mode == "Find"){
		$sql = $sql."and ".$Search." like '%".$SearchString."%' ";
	}
...
-->
              ...
              <div class="card-header">
				<div class="row">
					<div class="col-12">
						<a href="/rtsc_applyworship/beforeRegistWorshipFamily.php?worshipDate=2021-03-14"><button type="button" class="btn btn-primary"><i class="fas fa-users"></i> 예배신청하기(가족)</button></a>
						<a href="/rtsc_applyworship/registWorship.php?worshipDate=2021-03-14"><button type="button" class="btn btn-primary"><i class="fas fa-user"></i> 예배신청하기(개인)</button></a>
					</div>
					<div class="col-12">
						<p>&nbsp;</p>
						<p>※ 화면에 보이는 QR코드는 <u>좌측 번호로 임의로 생성</u>하였으며 단순히 보여드리기위함입니다.<p>
						<p>※ 중복등록 되는 부분도 이미 알고 있으며 모든 것은 교회의 정책과 방향이 결정된 이후에 진행될 예정입니다.<p>
					</div>
				</div>
              </div>
...
?>
					<tr align="center">
						...
						<td>
							<img src="http://chart.apis.google.com/chart?cht=qr&chs=177x177&chl=<?php echo $list[$i]['num']?>&choe=UTF-8&chld=H|0">
						</td>
