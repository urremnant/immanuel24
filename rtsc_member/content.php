...
?>
			...
			<div class="card card-primary card-outline">
			  <div class="card-body box-profile">
				<div class="text-center">
				  ...
				  ?>
							<img src="../upload/<?php echo $row['photofilename']?>" class="profile-user-img img-fluid img-circle" alt="User Image" width="60">
					  <?php
						}else{
					  ?>
				...
				?>
				<p></p>
				<h3 class="text-center"><?php echo "<b>".$row['korname']."</b> ".$row_korChurchPosition['korChurchPosition']?>
				<?php
					if ($row['teacherGubun']=="정교사"){
						echo " <span class='btn btn-success btn-sm'>".$row['teacherGubun']."</span>";
					}
					if ($row['teacherGubun']=="부교사"){
						echo " <span class='btn btn-warning btn-sm'>".$row['teacherGubun']."</span>";
					}
					if ($row['teacherGubun']=="인턴십교사"){
						echo " <span class='btn btn-info btn-sm'>".$row['teacherGubun']."</span>";
					}
				?>
				</h3>
				<?php
					if ($row['engname']<>""){
						echo "<h3 class='text-center'>".$row['engname']."</h3>";
					}
				?>
				<p></p>
				...
				<?php
					if ($row['tckYN'] == "Y"){
						echo " <span class='btn btn-danger btn-sm'>TCK</span>";
					}
					if ($row['pkYN'] == "Y"){
						echo " <span class='btn btn-success btn-sm'>PK</span>";
					}
					if ($row['mkYN'] == "Y"){
						echo " <span class='btn btn-warning btn-sm'>MK</span>";
					}
				?>
				</h3>
				<p></p>
				<div class="text-center">
					<?php
						if ($dataAccessType == "W"){
					?>
						<a class="btn btn-info btn-sm" href="edit.php?page=<?php echo $page?>&memberID=<?php echo $row['memberID'] ?>&churchareaCode=<?php echo $churchareaCode ?>&rtdept1Code=<?php echo $rtdept1Code ?>&rtdept2Code=<?php echo $rtdept2Code ?>&deptMoveCode=<?php echo $deptMoveCode ?>&mode=<?php echo $mode ?>&Search=<?php echo $Search?>&SearchString=<?php echo $SearchString?>"><i class="fas fa-pencil-alt"></i> Edit</a>
						<a class="btn btn-danger btn-sm" href="javascript:del()"><i class="fas fa-trash"></i> Delete</a>
					<?php
						}
					?>
						<a class="btn btn-info btn-sm" href="list.php?page=<?php echo $page?>&churchareaCode=<?php echo $churchareaCode ?>&rtdept1Code=<?php echo $rtdept1Code ?>&rtdept2Code=<?php echo $rtdept2Code ?>&deptMoveCode=<?php echo $deptMoveCode ?>&mode=<?php echo $mode ?>&Search=<?php echo $Search?>&SearchString=<?php echo $SearchString?>"><i class="far fa-list-alt"></i> List</a>
				</div>
				<p></p>
	...
	?>	
				  <li class="list-group-item">
					<i class="fa-solid fa-user-check"></i> <b>교인번호</b>
					<?php
						if ($row['myNo']<>""){
					?>
	...
	?>			 
				  ...
				  <li class="list-group-item">
					<i class="fas fa-building"></i> <b>학원 동아리 방과후교실</b>
				  </li>
				  <li class="list-group-item">
					<?php
						if ($row['afterschool']<>""){
				  ...
				  <li class="list-group-item">
					<i class="fas fa-layer-group"></i> <b>관심전문별</b>
					<?php
				  ...
				  <li class="list-group-item">
					<i class="fas fa-house-user"></i> <b>주소</b>
				  </li>
				  <li class="list-group-item">
					<?php
						if ($row['zipcode']<>""){
				  ...
				  <li class="list-group-item">
					<i class="fas fa-cubes"></i> <b>취미 특기 놀이</b>
				  </li>
				  <li class="list-group-item">
					<?php
						if ($row['hobby']<>""){
            ...
            <div class="card card-primary card-tabs">
              <div class="card-header p-0 pt-1">
                <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                  ...
                  <li class="nav-item">
                    <a class="nav-link" id="internship-tab" data-toggle="pill" href="#internship" role="tab" aria-controls="internship" aria-selected="false">인턴십</a>
                  </li>
                </ul>
              </div>
              <div class="card-body">
...
?>
							<?php
								$sql_timeline = "select a.idx, b.korname, b.photofilename, c.korChurchPosition, a.content, a.inputDate from member_timeline a, homepage_admin b, churchPosition c where a.homepage_admin_idx = b.homepage_admin_idx and b.churchPositionCode = c.churchPositionCode and a.memberID = '".$row['memberID']."' order by a.inputDate desc";
								//echo $sql_timeline;
...
?>
							...
							<div class="card-body table-responsive p-0">
<?php
			$sql_history = "select * from member_history where memberID = '".$memberID."' order by year desc";
...
?>
							<div class="card-body">
								<span class="btn btn-success btn-md">관심전문별 : <?php echo $row_expertMeeting['korProfessional']?></span>
							</div>
							<div class="card-header">
							...
							<div class="card-body table-responsive p-0">
<?php
		$sql_internship = "select korname, mobile, company from member where memberID like 'T%' and expertMeetingCode = '".$row['expertMeetingCode']."' and memberID <> '".$memberID."' order by korname";
...
?>
							...
							<div class="card-body table-responsive p-0">
<?php
		$sql_internship = "select korname, mobile, schoolinfo from member where memberID like 'R%' and expertMeetingCode = '".$row['expertMeetingCode']."' and memberID <> '".$memberID."' order by korname";
		$result_internship = mysqli_query($conn, $sql_internship);
?>
								<table class="table table-hover text-nowrap">
								<thead>
...
<?php
		...
		echo "관심 전문별을 선택하시면 해당 관심전문별을 선택한 교사와 렘넌트를 보실 수 있습니다. ";
		if ($dataAccessType == "W"){
?>
						<a class="btn btn-info btn-sm" href="edit.php?page=<?php echo $page?>&memberID=<?php echo $row['memberID'] ?>&churchareaCode=<?php echo $churchareaCode ?>&rtdept1Code=<?php echo $rtdept1Code ?>&rtdept2Code=<?php echo $rtdept2Code ?>&deptMoveCode=<?php echo $deptMoveCode ?>&mode=<?php echo $mode ?>&Search=<?php echo $Search?>&SearchString=<?php echo $SearchString?>"><i class="fas fa-pencil-alt"></i> 관심전문별 수정하기</a>
<?php
		}
	}
?>
