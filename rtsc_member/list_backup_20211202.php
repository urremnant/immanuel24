...
//	echo $sql;
	...
	$str = ''; 
	if ($page > 1) {
		$str .= '<li class="page-item"><a class="page-link"  href="list.php?page=1&mode='.$mode.'&Search='.$Search.'&SearchString='.$SearchString.'&churchareaCode='.$churchareaCode.'&rtdept1Code='.$rtdept1Code.'&rtdept2Code='.$rtdept2Code.'&deptMoveCode='.$deptMoveCode.'">처음</a></li>';
	}
	...
	if ($start_page > 1) $str .= '<li class="page-item"><a class="page-link" href="list.php?page='.($start_page-1).'&mode='.$mode.'&Search='.$Search.'&SearchString='.$SearchString.'&churchareaCode='.$churchareaCode.'&rtdept1Code='.$rtdept1Code.'&rtdept2Code='.$rtdept2Code.'&deptMoveCode='.$deptMoveCode.'">이전</a></li>';

	if ($total_page > 1) {
		for ($k=$start_page;$k<=$end_page;$k++) {
			if ($page != $k)
				$str .= '<li class="page-item"><a class="page-link" href="list.php?page='.$k.'&mode='.$mode.'&Search='.$Search.'&SearchString='.$SearchString.'&churchareaCode='.$churchareaCode.'&rtdept1Code='.$rtdept1Code.'&rtdept2Code='.$rtdept2Code.'&deptMoveCode='.$deptMoveCode.'">'.$k.'</a></li>';
			else
				$str .= '<li class="page-item"><a class="page-link" href="list.php?page='.$k.'&mode='.$mode.'&Search='.$Search.'&SearchString='.$SearchString.'&churchareaCode='.$churchareaCode.'&rtdept1Code='.$rtdept1Code.'&rtdept2Code='.$rtdept2Code.'&deptMoveCode='.$deptMoveCode.'"><font color="red"><b>'.$k.'</b></font></a></li>';
				// $str .= '<span class="current">'.$k.'</span>';
		}
	}

	if ($total_page > $end_page) $str .= '<li class="page-item"><a class="page-link" href="list.php?page='.($end_page+1).'&mode='.$mode.'&Search='.$Search.'&SearchString='.$SearchString.'&churchareaCode='.$churchareaCode.'&rtdept1Code='.$rtdept1Code.'&rtdept2Code='.$rtdept2Code.'&deptMoveCode='.$deptMoveCode.'">다음</a></li>';

	if ($page < $total_page) {
		$str .= '<li class="page-item"><a class="page-link" href="list.php?page='.$total_page.'&mode='.$mode.'&Search='.$Search.'&SearchString='.$SearchString.'&churchareaCode='.$churchareaCode.'&rtdept1Code='.$rtdept1Code.'&rtdept2Code='.$rtdept2Code.'&deptMoveCode='.$deptMoveCode.'">맨끝</a></li>';
	}
...
function select_deptMove(deptMoveCode){
	var checkdatalist="";
	...
	if (checkdatalist == "" ){
		...
		//alert(deptMoveCode);
		//alert(checkdatalist);
		window.open('deptMove.php?deptMoveCode='+deptMoveCode+'&checkdatalist='+checkdatalist,'deptMove','width=500 height=250, resizable=no');
	}
}
function select_deptMove_cancel(){
	var checkdatalist="";
	for(i=0; i < document.datalist.elements.length; ++i){
		if(document.datalist.elements[i].checked == true){
			checkdatalist=checkdatalist+document.datalist.elements[i].value+",";
	...
	if (checkdatalist == "" ){
		...
		//alert(checkdatalist);
		window.open('deptMove_cancle.php?checkdatalist='+checkdatalist,'deptMove_cancel','width=500 height=250, resizable=no');
	}
}
-->
</script>
			...
			?>
				  <div class="card-header">
						<div class="col-12">
							...
							<a href="javascript:select_deptMove('<?php echo $member_deptMoveCode;?>');" class="btn btn-primary">등반대상자로 설정하기</a>
							<a href="javascript:select_deptMove_cancel();" class="btn btn-primary">등반대상자 취소하기</a>
						</div>
				  </div>
			<?php
					}
...
?>
						<tr>
							...
							<td><a href="content.php?page=<?php echo $page?>&memberID=<?php echo $list[$i]['memberID'] ?>&churchareaCode=<?php echo $churchareaCode ?>&rtdept1Code=<?php echo $rtdept1Code ?>&rtdept2Code=<?php echo $rtdept2Code ?>&deptMoveCode=<?php echo $deptMoveCode ?>&mode=<?php echo $mode ?>&Search=<?php echo $Search?>&SearchString=<?php echo $SearchString?>"><?php echo $list[$i]['korname'] ?></a></td>
...
<?php if (count($list) == 0) { echo '<tr class="text-center"><td colspan="9"><font color="blue">해당 데이터가 없습니다.</font></td></tr>'; } ?>
					  </tfoot>
					</table>
				  </div>
				  <!-- /.card-body -->
