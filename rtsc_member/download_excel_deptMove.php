<?php
	...
	$sql_deptMove =  "select * from deptMove where deptMoveCode = '".$deptMoveCode."'";
	$result_deptMove = mysqli_query($conn, $sql_deptMove);
	$row_deptMove = mysqli_fetch_assoc($result_deptMove);
	
	$sql_deptFrom = "select rtdept1Name from rtdept1 where rtdept1Code = '".$row_deptMove['from_rtdept1Code']."'";
	$result_deptFrom = mysqli_query($conn, $sql_deptFrom);
	$row_deptFrom = mysqli_fetch_assoc($result_deptFrom);

	$sql_deptTo = "select rtdept1Name from rtdept1 where rtdept1Code = '".$row_deptMove['to_rtdept1Code']."'";
	$result_deptTo = mysqli_query($conn, $sql_deptTo);
	$row_deptTo = mysqli_fetch_assoc($result_deptTo);

	$excelname = "From".$row_deptFrom['rtdept1Name']."to".$row_deptTo['rtdept1Name'];
	...
	$sql = "select a.*, b.rtdept1Name, ifnull((select rtdept2Name from rtdept2 where rtdept2Code = a.rtdept2Code), '') as rtdept2Name, a.korname, c.korChurchPosition, d.korChurchAreaName, d.korParishName, e.korProfessional, f.korCountryName from member a, rtdept1 b, churchPosition c, churcharea d, expertMeeting e, country f where a.rtdept1Code = b.rtdept1Code and a.churchPositionCode = c.churchPositionCode and a.churchareaCode = d.churchareaCode and a.expertMeetingCode = e.expertMeetingCode and a.countryCode = f.countryCode and a.deptMoveCode='".$deptMoveCode."' order by rtdept1Code, rtdept2Code, memberID, korname";
//	echo $sql;
