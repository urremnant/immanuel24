<?php
	...
	//로그기록
	$sql_log = "insert into logData(homepage_admin_idx, gubun, excelname, loginDate) values ('".$_SESSION['ss_homepage_admin_idx']."', 'excel', '".$excelname."', now())";
...
?>
	<tr align="center">
		...
		<td>
			<?php 
				$sql_timeline = "select b.korname, c.korChurchPosition, a.content, a.inputDate from member_timeline a, homepage_admin b, churchPosition c where a.homepage_admin_idx = b.homepage_admin_idx and b.churchPositionCode = c.churchPositionCode and a.memberID = '".$row['memberID']."' order by a.inputDate";
				$result_timeline = mysqli_query($conn, $sql_timeline);
				while ($row_timeline = mysqli_fetch_assoc($result_timeline)) {
					echo $row_timeline['korname'].$row_timeline['korChurchPosition']."/".$row_timeline['inputDate']."/".$row_timeline['content']."<br style='mso-data-placement:same-cell;'>";
				}
			?>
		</td>
	</tr>
