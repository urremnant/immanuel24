...
//	D10012	사랑부		
	...
	$sql = "select a.memberID, a.photofilename, d.korParishName, b.rtdept1Name, ifnull((select rtdept2Name from rtdept2 where rtdept2Code = a.rtdept2Code), '') as rtdept2Name, a.korname, c.korChurchPosition, a.mobile from member a, rtdept1 b, churchPosition c, churcharea d where a.rtdept1Code = b.rtdept1Code and a.churchPositionCode = c.churchPositionCode and a.churchareaCode = d.churchareaCode "; // 회원 정보를 조회
	if ($churchareaCode <> ""){
		$sql = $sql."and a.churchareaCode = '".$churchareaCode."' "; 
	}
	if ($rtdept1Code <> ""){
	...
	$sql = $sql."ORDER BY a.korname LIMIT {$from_record}, {$page_rows}";
//	echo $sql;
	$result = mysqli_query($conn, $sql);
...
//								alert(rtdept2Code);
							...
							?>
									<option value="A99999">미분류</option>
								</select>
							</div>
